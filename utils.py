...
def find_files(filenames, *neccesary_conditions):
    result = []
    for filename in filenames:
        flag = True
        for condition in neccesary_conditions:
            if condition.replace("-", "_") not in filename.replace("-", "_"):
                flag = False
                break
        if flag:
...
def build_confusion_matrix(diagrams, pairwise_distance, keys_a, keys_b):
    n = len(keys_a)
    m = len(keys_b)
    distances = np.zeros((n, m))
    for i, key_a in enumerate(keys_a):
        if diagrams[key_a] is not None:
            bars_a = diagrams[key_a].reshape(1, -1, 3)
            pairwise_distance.fit(bars_a)
        for j, key_b in enumerate(keys_b):
            if diagrams[key_a] is None or diagrams[key_b] is None:
                distances[i, j] = np.nan
            else:
                bars_b = diagrams[key_b].reshape(1, -1, 3)
                distances[i, j] = pairwise_distance.transform(bars_b)[0, 0]
    return distances
...
def calc_min_max_total_dists(diagrams, 
                             pairwise_distance, 
                             fixed_param_a=None, 
                             fixed_param_b=None, 
                             vary_params=None):
    if fixed_param_a is None or fixed_param_b is None or vary_params is None:
        return
    min_val = 1000
    max_val = -1
    total_n = 0
    for i, vary_param_a in enumerate(vary_params):
        ...
        for j, vary_param_b in enumerate(vary_params):
            ...
            distances = build_confusion_matrix(diagrams, 
                                               pairwise_distance, 
                                               filenames_a, 
                                               filenames_b)
            if len(distances.reshape(-1)) == 0:
                continue
            total_n += 1
            min_val = min(min_val, np.min(distances))
            max_val = max(max_val, np.max(distances))
    print("Bounds:", min_val, max_val)
    return min_val, max_val, total_n
...
def build_grouped_conf_matrix(diagrams, pairwise_distance, *order):
    k = len(list(itertools.product(*order)))
#     print("Total", k)
    avg_distances = np.zeros((k, k))
    for i, combination_one in enumerate(itertools.product(*order)):
#         print(i)
        filenames_one = find_files(diagrams.keys(), *combination_one)
#         print(combination_one, filenames_one)
        for j, combination_two in enumerate(itertools.product(*order)):
            filenames_two = find_files(diagrams.keys(), *combination_two)
#             print(combination_two, filenames_two)
            conf_matrix = build_confusion_matrix(diagrams, pairwise_distance, filenames_one, filenames_two)
            avg_distances[i, j] = np.nanmean(conf_matrix)
